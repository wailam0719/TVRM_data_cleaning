...
X,y = data.load_excel_from_dir("C:/data/plates.xlsx")
"""
...
class Data:
    ...
    @classmethod
    def parsing_dataframe(cls,filename):
    ...
    def load_csv(cls,filename):
        ...
        cls.x = cls.x.append(cls.auction[Data.x_list],ignore_index = False)
        cls.x = cls.y.append(cls.auction[['price']], ignore_index = False)
        return cls.x,cls.y



    @classmethod
    def load_excel(cls,filename):
        date_str = os.path.basename(filename).strip('.xlsx')
        date = pd.to_datetime(date_str)
        Data.cleanning_auction(filename,date)
        cls.x = cls.x.append(cls.auction[Data.x_list],ignore_index = False)
        cls.y = cls.y.append(cls.auction[['price']], ignore_index = False)
        return cls.x,cls.y


    @classmethod
    def load_csv_from_dir(cls,path):
        filepath = []
        ...
        for file in filepath:
            ...
            date = pd.to_datetime(date_str)
            cls.cleanning_auction(file,date)
            cls.x = cls.x.append(cls.auction[Data.x_list],ignore_index = False)
            cls.y = cls.y.append(cls.auction[['price']], ignore_index = False)
        return cls.x,cls.y

    @classmethod
    def load_excel_from_dir(cls,path):
        filepath = []
        for root, dirs, files in os.walk(path):
            for file in files:
